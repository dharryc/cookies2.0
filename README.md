...
> Users can log in and view their family(ies) through a navbar. When they view an individual family, items that other users have made visible in that family will be visable, as well as a toggleable button to mark an item as purchased. The user can also navigate to their item page, which will allow them to add a new item to their list, then change the visibility to include or exclude visibility across families that the user is a member of.
